...
int StructureManager::destroyStructure(StructureObject* structureObject, bool playEffect, String reason) {
	...
	if (creature != nullptr && !reason.isEmpty()) {
		ManagedReference<ReckoningLogManager*> logMan = creature->getZoneServer()->getReckoningLogManager();

		if (logMan != nullptr) {
			StringBuffer logEntry;
			logEntry << "Structure ID: " << structureObject->getObjectID() << " Owned By: " << creature->getFirstName() << " has been destroyed because " << reason;
			logMan->logAction(LogType::STRUCTUREDESTROY, logEntry.toString());
		}
	}
...
void StructureManager::reportStructureStatus(CreatureObject* creature,
	...
	if (structure->isBuildingObject()) {
		...
		String items = String::valueOf(building->getCurrentNumberOfPlayerItems());
		String totalItems = String::valueOf(building->getMaximumNumberOfPlayerItems());

		status->addMenuItem("@player_structure:items_in_building_prompt " + items + "/" + totalItems);
...
void StructureManager::withdrawMaintenance(StructureObject* structure, CreatureObject* creature, int amount) {
	if (!structure->isGuildHall() && !structure->isOwnerOf(creature)) {
		return;
	}
...
int StructureManager::packupStructure(CreatureObject* creature) {
	...
	if (structureObject->isRedeedable()) {
		...
		if (datapad->isContainerFullRecursive()) {
			...
			StringBuffer logEntry;
			logEntry << "Structure ID: " << structureObject->getObjectID() << " Owned By: " << creature->getFirstName() << " has been packed up by the player.";
			logMan->logAction(LogType::STRUCTUREPACKUP, logEntry.toString());
...
void StructureManager::systemPackupStructure(StructureObject* structureObject, CreatureObject* creature) {
	...
	if (datapad == nullptr) {
		...
		StringBuffer logEntry;
		logEntry << "Structure ID: " << structureObject->getObjectID() << " Owned By: " << creature->getFirstName() << " has been packed up by the system.";
		logMan->logAction(LogType::SYSTEMSTRUCTUREPACKUP, logEntry.toString());
	}
}
