	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
		...
		try {
			...
			if (commandType.beginsWith("createitem")) {
				...
				bool area = false;
				float range = 64;
				String crafter = "System Generated";
				...
				if (area) {
					...
					CloseObjectsVector* closeObjectsVector = (CloseObjectsVector*) creature->getCloseObjects();
					if (closeObjectsVector == nullptr) {
						creature->getZone()->getInRangeObjects(creature->getPositionX(), creature->getPositionY(), range, &closeObjects, true);
					} else {
						closeObjectsVector->safeCopyTo(closeObjects);
					}
					...
					for (int i = 0; i < closeObjects.size(); i++) {
						...
						if (targetObject->isPlayerCreature() && creature->isInRange(targetObject, range)) {
							...
							object->setCraftersName(crafter);
							...
							if (inventory->transferObject(object, -1, true)) {
								...
								logEntry << creature->getFirstName() << " on account: " << ghost->getAccountID() << " used the /object giveobject command to give the object: " <<
					...
					ManagedReference<SceneObject*> inventory = targetCreature->getSlottedObject("inventory");
					if (inventory == nullptr || inventory->isContainerFullRecursive())
						return GENERALERROR;
					...
					if (creature->getFirstName() == "Santa" && ghost->getAccountID() == 23)
						crafter = "Santa's Elves";
					...
					if (inventory->transferObject(object, -1, true)) {
						...
						StringBuffer logEntry;
						logEntry << creature->getFirstName() << " on account: " << ghost->getAccountID() << " used the /object giveobject command to give the object: " <<
								object->getDisplayedName() << " to " << targetCreature->getFirstName() << " on account: " << targetGhost->getAccountID() << ".";
						logMan->logAction(LogType::STAFFTRADE, logEntry.toString());
			...
			creature->sendSystemMessage("SYNTAX: /object giveitem <objectTemplatePath> OR /object giveitem <objectTemplatePath> <-area range>");
