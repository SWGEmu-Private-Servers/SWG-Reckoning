...
BaseMessage* SuiETerminalBoxImplementation::generateMessage() {
	...
	for (int i = 0; i < currentNode->getChildNodeSize(); ++i) {
		ETerminalMenuNode* node = currentNode->getChildNodeAt(i);
...
ETerminalMenuNode* SuiETerminalBoxImplementation::getCurrentNode() {
	return currentNode;
}

void SuiETerminalBoxImplementation::setCurrentNode(ETerminalMenuNode* node) {
	currentNode = node;
}
